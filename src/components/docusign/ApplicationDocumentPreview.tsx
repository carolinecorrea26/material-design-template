...
import { isSectionVisible } from "../form/FormRoutePage";
...
const documentPages: Array<{ pageId: PageId; title: string }> = [
  ...
  { pageId: "coverage-questions", title: "Coverage Details" },
  { pageId: "coverage-options", title: "Coverage Elections" },
  { pageId: "beneficiary", title: "Beneficiaries" },
  { pageId: "contact", title: "Contact Information" },
  { pageId: "personal", title: "Personal Information" },
  { pageId: "financial", title: "Financial Information" },
...
function buildCoverageOptionsSection(
  ...
  Object.entries(amounts).forEach(([compoundKey, rawValue]) => {
    ...
    entries.push({
      label: `${coverage.name} - ${getApplicantLabel(applicant)} amount`,
      value: formatFieldValue("average-monthly-income", rawValue),
    });
  });

  Object.entries(riders).forEach(([compoundKey, rawValue]) => {
    if (!rawValue) {
      return;
    }

    const [coverageId, riderId, applicant] = compoundKey.split(":");
    const coverage = coverages.find((item) => item.id === coverageId);
    const rider = coverage?.riders?.find((item) => item.id === riderId);

    if (!coverage || !rider) {
      return;
    }

    const riderAmount = riderAmounts[compoundKey];
    entries.push({
      label: `${coverage.name} - ${getApplicantLabel(applicant)} rider`,
      value:
        rider.hasAmount && riderAmount != null
          ? `${rider.name} (${formatFieldValue("average-monthly-income", riderAmount)})`
          : rider.name,
    });
  });

  Object.entries(waitingPeriods).forEach(([coverageId, rawValue]) => {
    const coverage = coverages.find((item) => item.id === coverageId);
    const option = coverage?.waitingPeriodOptions?.find(
    ...
    entries.push({
      label: `${coverage.name} waiting period`,
      value: option?.label ?? String(rawValue ?? PLACEHOLDER),
    });
  });

  Object.entries(maxBenefitPeriods).forEach(([coverageId, rawValue]) => {
    const coverage = coverages.find((item) => item.id === coverageId);
    const option = coverage?.maxBenefitPeriodOptions?.find(
      (item) => item.value === rawValue,
    );
    ...
    entries.push({
      label: `${coverage.name} max benefit period`,
      value: option?.label ?? String(rawValue ?? PLACEHOLDER),
    });
  });
    ...
    groups: [
      {
        entries: entries.length
          ? entries
          : [{ label: "Coverage elections", value: PLACEHOLDER }],
      },
    ],
  };
}
...
function buildDocumentSections(values: ApplicationFormValues) {
  return documentPages
    .map((entry) => {
      ...
      if (entry.pageId === "coverage-options") {
        return buildCoverageOptionsSection(values);
      }
