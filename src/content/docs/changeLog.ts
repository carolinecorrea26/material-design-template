...
export const changeLog: ChangeLogEntry[] = [
  ...
  },
  {
    ...
    details:
      ...
      "Inventory updates: src/content/docs/componentInventory.ts was updated with the verified generated story IDs for exactly these 20 components, each checked against the actual Storybook build output; components without stories remained unavailable. src/docs/ComponentInventory.md's Story?/Action columns updated to Yes/Done for the same 20 rows. src/docs/StorybookAudit.md gained a full §11 write-up of what was built, the two fixes, and a recommended next phase (Navigation first, then Overlays once the QuoteModal dead-code question is resolved with the team, then Coverage & Commerce as a themed sub-pass given how tightly coupled those 15 components are). " +
