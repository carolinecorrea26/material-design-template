...
- `src/config/coverageConstants.ts` — `getCategoryRequirements`, `getMaxAggregateNotes` — business-rule resolvers that read `ClientConfig` state directly (e.g. `clientMaxAggregateNoteOverrides`, `hideSmokerQuestion`) but live outside `types.ts`/`clients/*.ts`.
...
| D4 | **Coverage business rules split across three places** | A single effective coverage-page behavior (e.g. "does this client see the smoker question") is assembled from `ClientConfig.coverages.hideSmokerQuestion` (client override, typed), `coverageConstants.ts::getCategoryRequirements` (logic, untyped relationship to the override), and `configurations.ts` (prose description, hand-maintained, already flags the FUW/WAEPA quote-tool exception in free text rather than as data). |
