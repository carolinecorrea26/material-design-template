...
export const handlers = [
  http.get(`${API_BASE}/products`, () => {
    return HttpResponse.json(products as Product[]);
  }),
  ...
  http.post(`${API_BASE}/rate/quote`, async ({ request }) => {
    ...
    const base = (rates?.base as Record<string, number>)?.[body.productId] ?? 0.1;

    const monthly = round2(
      base * (body.amount / 1000) * nicotineFactor(body.smoker) * ageBandFactor(body.age)
    );
