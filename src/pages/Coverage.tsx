...
export default function Coverage() {
  ...
  const [amountByKey, setAmountByKey] = React.useState<
    Record<SelKey, number | "">
  >({});
  const [rateByKey, setRateByKey] = React.useState<Record<SelKey, number>>({});
  const [selectedByKey, setSelectedByKey] = React.useState<
    Record<SelKey, boolean>
  >({});
  const [optionalBenefitsByKey, setOptionalBenefitsByKey] = React.useState<
    Record<SelKey, Record<string, boolean>>
  >({});
  ...
  const selfCats = React.useMemo(
    () => new Set(elig?.selfCoverages ?? []),
    [elig?.selfCoverages],
  );
  const spouseCats = React.useMemo(
    () => new Set(elig?.spouseCoverages ?? []),
    [elig?.spouseCoverages],
  );
  ...
  }, [activeCategoryIndex, visibleCategories.length]);

  React.useEffect(() => {
    if (location.pathname === "/coverage") {
      setActiveCategoryIndex(0);
    }
  }, [location.pathname]);
    ...
    if (products.length > 0) {
      ...
      products.forEach((p) => {
        chosenApplicants.forEach((app) => {
          ...
          if (app === "self" && selfCats.size && !selfCats.has(p.category))
            return;
          if (
            app === "spouse" &&
            spouseCats.size &&
            !spouseCats.has(p.category)
          )
            return;
        ...
        if (amount !== "") {
          ...
          if (product) {
            const smoker =
              applicant === "self"
                ? elig?.smokerSelf === "yes"
                : elig?.smokerSpouse === "yes";
            const quotePromise = quoteRate({
              productId,
              applicant,
              amount,
              smoker,
      ...
      products.forEach((product) => {
        chosenApplicants.forEach((applicant) => {
          ...
          if (
            applicant === "self" &&
            selfCats.size &&
            !selfCats.has(product.category)
          )
            return;
          if (
            applicant === "spouse" &&
            spouseCats.size &&
            !spouseCats.has(product.category)
          )
            return;
  ...
  const isLastCategory = activeCategoryIndex === visibleCategories.length - 1;
  ...
  if (loading) {
    return (
      <FormPageLayout header={<PageHeader title="Coverage" />}>
        <Card>
          <CardContent>
  ...
  const anyVisible = products.length > 0;

  if (!anyVisible) {
    return (
      <FormPageLayout header={<PageHeader title="Coverage" />}>
        <Alert severity="info">
          No eligible products were found for your current selections. Please
          adjust your choices on the Eligibility page.
        </Alert>
        <Stack direction="row" justifyContent="flex-end">
          <Button variant="outlined" onClick={() => navigate("/eligibility")}>
            Back to Eligibility
          </Button>
        </Stack>
      </FormPageLayout>
    ...
    <FormPageLayout
      ...
      navigation={
        <PageNavigation
          ...
          continueText={isLastCategory ? "Continue" : "Next"}
          hasUnsavedChanges={() => Object.values(selectedByKey).some(Boolean)}
        />
      }
    >
      <FormStepTransition triggerKey={activeCategoryIndex}>
        {submitAttempted &&
          !Object.values(selectedByKey).some(
            (selected) => selected === true,
          ) && (
            <Alert severity="error">
              You must make a coverage selection. Please check at least one
              product below.
            </Alert>
          )}
        ...
        {activeCategory &&
          (() => {
            const categoryProducts =
              productsByCategory[activeCategory as CoverageCategory] ?? [];
            const visibleProducts = categoryProducts.filter((p) => {
              const allowedApplicants = chosenApplicants.filter((a) => {
                ...
                if (a === "self" && selfCats.size && !selfCats.has(p.category))
                  return false;
                if (
                  a === "spouse" &&
                  spouseCats.size &&
              ...
              <Box sx={{ mb: 4 }}>
                ...
                >
                  <Typography variant="h4" sx={{ fontWeight: 600 }}>
                    {getCoverageLabel(activeCategory as CoverageCategory)}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    {activeCategoryIndex + 1} of {visibleCategories.length}
                  </Typography>
                </Stack>

                {visibleProducts.length === 0 ? (
                  <Alert severity="info">
                    No eligible products are available in this category based on
                    your selections.
                  </Alert>
                ) : (
                  <Stack spacing={3}>
                          ...
                          <AccordionDetails>
                            <Stack spacing={2}>
                              ...
                              {p.name === "Term Life Insurance" && (
                                <Alert
                                  severity="info"
                                  sx={{ ...commonStyles.infoAlert, mb: 2 }}
                                >
                                  <Typography variant="body2">
                                    Annual Renewable Group Term Life, designed
                                    to provide protection for both you and your
                                    family. Optional Chronic Illness Rider (CIR)
                                    available.
                                  </Typography>
                                  <Typography variant="body2" sx={{ mt: 1 }}>
                                    QuickDecision<sup>SM</sup> allows you to get
                                    a faster decision on your application with
                                    typically no medical exam using medical
                                    underwriting automated processing.
                                    Eligibility factors include coverage amount,
                                    age, and state availability. If
                                    QuickDecision is not available, you may
                                    apply on a standard underwritten basis.{" "}
                                    <Button
                                      variant="text"
                                      size="small"
                                      onClick={() =>
                              ...
                              {p.name ===
                                ...
                                >
                                  <Typography variant="body2">
                                    Rates for coverage are expected to remain
                                    level but are not guaranteed.
                                  </Typography>
                                  <Typography variant="body2" sx={{ mt: 1 }}>
                                    QuickDecision<sup>SM</sup> allows you to get
                                    a faster decision on your application with
                              ...
                              {p.name ===
                                ...
                                >
                                  <Typography variant="body2">
                                    Great fit for those in their 30s and 40s! A
                                    20 year term can bring stability to your
                                    financial planning.
                                  </Typography>
                                  <Typography variant="body2" sx={{ mt: 1 }}>
                                    QuickDecision<sup>SM</sup> allows you to get
                              ...
                              {p.name ===
                                ...
                                >
                                  <Typography variant="body2">
                                    Accidents are unpredictable. This guaranteed
                                    acceptance coverage can help you safeguard
                                    your family finances should an accident
                                    occur.
                                  </Typography>
                                </Alert>
                              )}
                              ...
                              {p.name ===
                                ...
                                >
                                  <Typography variant="body2">
                                    Benefits are paid directly to you for
                                    disabilities due to a covered sickness or
                                    injury up to age 65. Various additional
                                    features that could be beneficial to you
                                    exist with this coverage.
                                  </Typography>
                                  <Typography variant="body2" sx={{ mt: 1 }}>
                                    QuickDecision<sup>SM</sup> allows you to get
                                    a faster decision on your application with
                              ...
                              {(p.name === "Hospital Money Insurance" ||
                                p.name === "Hospital Income Insurance") && (
                                <>
                                  <Alert
                                    severity="info"
                                    sx={{ ...commonStyles.infoAlert, mb: 2 }}
                                  >
                                    <Typography variant="body2">
                                      Guaranteed coverage to help offset costs
                                      during a hospital stay. Benefits paid
                                      directly to you to use where you need the
                                      most. Coverage is subject to a
                                      pre-existing condition limitation. Please
                                      see the brochure for detail.
                                    </Typography>
                                    <Typography
                                      variant="body2"
                                    ...
                                    >
                                      ...
                                      AND IS NOT A SUBSTITUTE FOR MAJOR MEDICAL
                                      COVERAGE.
                                    </Typography>
                                  </Alert>
                                  ...
                                  >
                                    ...
                                    >
                                      IMPORTANT: This is a fixed indemnity
                                      policy,
                                      <br />
                                    ...
                                    <Typography
                                      variant="body2"
                                      sx={{ fontSize: "14pt", mb: 1 }}
                                    >
                                      This fixed indemnity policy may pay you a
                                      limited dollar amount if you're sick or
                                      hospitalized. You're still responsible for
                                      paying the cost of your care.
                                    </Typography>
                                    <Typography
                                      component="ul"
                                      variant="body2"
                                      sx={{ fontSize: "14pt", mb: 1, pl: 3 }}
                                    >
                                      <li>
                                        The payment you get isn't based on the
                                        size of your medical bill.
                                      </li>
                                      <li>
                                        There might be a limit on how much this
                                        policy will pay each year.
                                      </li>
                                      <li>
                                        This policy isn't a substitute for
                                        comprehensive health insurance.
                                      </li>
                                      <li>
                                        Since this policy isn't health
                                        insurance, it doesn't have to include
                                    ...
                                    >
                                      Looking for comprehensive health
                                      insurance?
                                    </Typography>
                                    <Typography
                                      component="ul"
                                    ...
                                    >
                                      <li>
                                        ...
                                        1-800-318-2596 (TTY: 1-855-889-4325) to
                                        find health coverage options.
                                      </li>
                                      <li>
                                        To find out if you can get health
                                        insurance through your job, or a family
                                    ...
                                    <Typography
                                      component="ul"
                                      variant="body2"
                                      sx={{ fontSize: "14pt", pl: 3 }}
                                    >
                                      <li>
                                        For questions or complaints about this
                                        policy, contact your State Department of
                                        Insurance. Find their number on the
                                        National Association of Insurance
                                        Commissioners' website (naic.org) under
                                        "Insurance Departments."
                                      </li>
                                      <li>
                                        If you have this policy through your
                                        job, or a family member's job, contact
                                        the employer.
                                      </li>
                                    </Typography>
                                  </Box>
                                        ...
                                        <Stack spacing={2}>
                                          ...
                                          {p.category === "LI" &&
                                            ...
                                            app === "self" && (
                                              ...
                                              >
                                                <Typography variant="body2">
                                                  The maximum available through
                                                  New York Life Insurance
                                                  Company for any individual is
                                                  $2,000,000, whether coverage
                                                  is in one or divided among
                                          ...
                                          {p.category === "LI" &&
                                            ...
                                            app === "spouse" && (
                                              ...
                                              >
                                                <Typography variant="body2">
                                                  Spouse coverage cannot exceed
                                                  member coverage (including in
                                                  force or requested coverage).
                                                </Typography>
                                              </Alert>
                                            )}
                                          ...
                                          {p.name ===
                                            ...
                                            app === "self" && (
                                              ...
                                              >
                                                <Typography variant="body2">
                                                  The maximum available through
                                                  all ABE Group Insurance
                                                  underwritten by New York Life
                                                  Insurance Company is $12,000
                                                  for a member whether coverage
                                                  is in one or divided among
                                                  several group policies.
                                                </Typography>
                                              </Alert>
                                          ...
                                          {p.name ===
                                            ...
                                            app === "spouse" && (
                                              ...
                                              >
                                                <Typography variant="body2">
                                                  Spouse coverage cannot exceed
                                                  nine times member coverage.
                                                  The maximum available through
                                                  all ABE Group Insurance
                                                  underwritten by New York Life
                                                  Insurance Company is $5,000
                                                  for a spouse, whether coverage
                                                  is in one or divided among
                                                  several group policies.
                                                </Typography>
                                              </Alert>
                                          ...
                                          val !== "" ? (
                                            ...
                                            <Box
                                              sx={
                                                commonStyles.pricingPlaceholder
                                              }
                                            >
                                              <Typography
                                                variant="h6"
                                                color="text.secondary"
                                                sx={
                                                  commonStyles.pricingPlaceholderText
                                                }
                                              >
                                            ...
                                            (app === "self" ||
                                              ...
                                              <Box sx={{ mt: 2, mb: 2 }}>
                                                ...
                                                <Typography
                                                  ...
                                                  sx={{ mt: 1, ml: 4 }}
                                                >
                                                  Surgical Option: Pay a
                                                  specified amount for a
                                                  qualifying surgical procedure
                                                  up to the maximum for the
                                                  option selected. See
                                                  brochure/certificate for
                                                  exclusions. Note: Selection
                                                  applies to all applicants for
                              ...
                              {isAMA && (
                                <Card
                                  variant="outlined"
                                  sx={{ ...commonStyles.coverageCard, mt: 2 }}
                                >
                                  <CardContent>
      ...
      >
        <DialogTitle>
          About QuickDecision<sup>SM</sup>
        </DialogTitle>
        <DialogContent>
          <DialogContentText>
            QuickDecision uses data sources to verify your health history, so
            there are typically no medical exams or lab tests to receive a
            decision on your insurance application. Any approval decisions on
            applications are conditional upon confirmation of your group status
            and coverage limit eligibility. A referral decision may be given if
            an underwriter needs to further review your application and may
            reach out to confirm details or request additional information.
