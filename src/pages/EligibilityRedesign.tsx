...
const getApplicantChipStyles = (applicant: string) => {
  switch (applicant) {
    case "Self":
      return { bgcolor: "rgba(25, 118, 210, 0.12)", color: "primary.main", borderColor: "rgba(25, 118, 210, 0.3)" };
    case "Spouse":
      return { bgcolor: "rgba(156, 39, 176, 0.08)", color: "secondary.dark", borderColor: "rgba(156, 39, 176, 0.25)" };
    case "Child":
      return { bgcolor: "rgba(76, 175, 80, 0.1)", color: "success.dark", borderColor: "rgba(76, 175, 80, 0.25)" };
    default:
...
const COVERAGE_CARDS = [
  {
    ...
    description: "Protect your family or business with level term and AD&D options tailored for attorneys.",
    icon: VolunteerActivismRounded,
    products: [
      { id: "li-term10", name: "10-Year Level Term Life Insurance", applicants: ["Self", "Spouse"], monthlyEstimate: "$20–$45/mo", coverageHighlight: "$500K of protection" },
      { id: "li-term20", name: "20-Year Level Term Life Insurance", applicants: ["Self", "Spouse"], monthlyEstimate: "$32–$60/mo", coverageHighlight: "$1M of protection" },
      { id: "li-adt", name: "Accidental Death & Dismemberment", applicants: ["Self", "Spouse", "Child"], monthlyEstimate: "$5–$12/mo", coverageHighlight: "$300K AD&D benefit" }
    ]
  },
  {
    id: "DI",
    title: "Group Disability Insurance",
    description: "Replace lost income and keep your practice running if an illness or injury sidelines you.",
    icon: AccessibleForwardRounded,
    products: [
      { id: "di-basic", name: "LTD 70% Monthly Benefit", applicants: ["Self", "Spouse"], monthlyEstimate: "$55–$110/mo", coverageHighlight: "70% income replacement" },
      { id: "di-student", name: "Resident & Fellow Disability", applicants: ["Self"], monthlyEstimate: "$28–$65/mo", coverageHighlight: "$5K/mo benefit" },
      { id: "di-premium", name: "Own-Occupation Disability", applicants: ["Self", "Spouse"], monthlyEstimate: "$80–$150/mo", coverageHighlight: "$12K/mo own-occ benefit" }
    ]
  },
  {
    id: "OO",
    title: "Office Overhead Expense",
    description: "Cover rent, payroll, and other business expenses during a prolonged disability.",
    icon: ApartmentRounded,
    products: [
      { id: "oo-10k", name: "$10K Monthly Expense Benefit", applicants: ["Self"], monthlyEstimate: "$65–$95/mo", coverageHighlight: "$10K office overhead" },
      { id: "oo-20k", name: "$20K Monthly Expense Benefit", applicants: ["Self"], monthlyEstimate: "$110–$160/mo", coverageHighlight: "$20K office overhead" }
    ]
  },
  {
    id: "SH",
    title: "Supplemental Health",
    description: "Cash benefits to offset hospital stays, critical illness, or recovery costs.",
    icon: LocalHospitalRounded,
    products: [
      { id: "sh-hospital", name: "Hospital Income Plan", applicants: ["Self", "Spouse", "Child"], monthlyEstimate: "$18–$35/mo", coverageHighlight: "$300/day hospital cash" },
      { id: "sh-critical", name: "Critical Illness Benefit", applicants: ["Self", "Spouse"], monthlyEstimate: "$22–$48/mo", coverageHighlight: "$50K lump sum" }
    ]
  }
] as const;
...
export default function Eligibility() {
  ...
  const coverageSelections = useWatch({ control: methods.control, name: "coverageProductSelections" }) as string[] | undefined;
  ...
  const handleToggleProduct = React.useCallback((productId: string) => {
    ...
    const next = current.includes(productId)
      ...
      : [...current, productId];
    methods.setValue("coverageProductSelections", next, { shouldDirty: true });
  ...
  React.useEffect(() => {
    const handleFillForm = () => {
      ...
      const filledData: EligibilityForm = {
        ...
        coverageProductSelections: ["li-term10", "di-basic", "oo-10k", "sh-hospital"]
      };
      ...
      <form onSubmit={methods.handleSubmit(onSubmit)} noValidate>
        <Stack spacing={2}>
          <PageHeader 
            ...
            icon={<PolicyOutlined sx={{ fontSize: 32, color: 'primary.main' }} />}
            animatedIcon
          />

          <Stack spacing={1.5} direction={{ xs: "column", md: "row" }} alignItems={{ md: "center" }} justifyContent="flex-end" gap="8px">
              ...
              <Stack spacing={4}>
                {COVERAGE_CARDS.map((card, index) => {
                  ...
                  const visibleProducts = card.products.filter(product => {
                    return applicantFilter.length === 0 || product.applicants.some(applicant => applicantFilter.includes(applicant));
                  });
                      ...
                      <Collapse in={isExpanded} timeout="auto" unmountOnExit>
                        <FormGroup>
                          {visibleProducts.map(product => {
                            const selected = (coverageSelections ?? []).includes(product.id);
                            return (
                              <FormControlLabel
                                key={product.id}
                                control={
                                ...
                                label={
                                  <Box>
                                    ...
                                    >
                                      ...
                                      >
                                        {product.applicants.map(applicant => {
                                          const chipStyles = getApplicantChipStyles(applicant);
                                          return (
                                            <Chip
                                              key={applicant}
                                              label={applicant}
                                    ...
                                    {(product.coverageHighlight || product.monthlyEstimate) && (
                                      <Typography variant="caption" color="text.secondary" sx={{ mt: 0.5, display: 'block' }}>
                                        {product.coverageHighlight ? `${product.coverageHighlight} for ${product.monthlyEstimate}` : product.monthlyEstimate}
                                      </Typography>
                                    )}
                                  </Box>
                                }
