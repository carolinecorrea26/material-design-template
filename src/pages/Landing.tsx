...
import { getClientBranding, getClientFeatures, ACTIVE_CLIENT_ID } from "../config/clients";
...
const HeroSection: React.FC<{ hideNonHero: boolean; hideQuote: boolean; scrollToQuote: () => void }> = ({ 
  ...
  return (
    ...
    >
      <Container maxWidth="lg" sx={{ maxWidth: '1400px !important', position: 'relative', zIndex: 1, pt: { xs: '260px', md: 0 } }}>
        <Box sx={{ 
          background: { xs: 'rgba(255, 255, 255, 0.85)', md: 'rgba(255, 255, 255, 0.95)' },
...
const QuoteForm: React.FC<{
  ...
  coverageType, setCoverageType, birthday, setBirthday, state, setState,
  gender, setGender, usesNicotine, setUsesNicotine, hoursPerWeek, setHoursPerWeek,
  monthlyIncome, setMonthlyIncome, onSubmit, quoteRef
}) => {
  return (
    <Box sx={{ py: 2 }}>
          ...
          <Card elevation={2} sx={{ borderLeft: 6, borderColor: 'primary.main' }}>
            <CardContent sx={{ p: { xs: 2, md: 4 } }}>
              <Stack spacing={3}>
                  ...
                  <Stack direction="row" spacing={2} alignItems="center">
                    <Typography 
                      ...
                      sx={{ 
                        ...
                        color: coverageType === 'life' ? 'primary.main' : 'text.secondary'
                      }}
                    >
                      Group Life
                    </Typography>
                    <Switch 
                      ...
                      onChange={(e) => setCoverageType(e.target.checked ? 'disability' : 'life')}
                      sx={{
                    ...
                    <Typography 
                      ...
                      sx={{ 
                        ...
                        color: coverageType === 'disability' ? 'primary.main' : 'text.secondary'
                      }}
                    >
                      Group Disability
                    </Typography>
                ...
                <Box sx={{ display: 'flex', justifyContent: { xs: 'center', md: 'flex-end' }, pt: 1 }}>
                  <Button 
                    variant="contained" 
                    size="large" 
                    onClick={onSubmit}
      ...
      {[...Array(3)].map((_, setIndex) => (
        <React.Fragment key={setIndex}>
          {RATINGS.map((item, idx) => (
            <Box key={idx} sx={{ display: 'flex', alignItems: 'center', gap: 1, whiteSpace: 'nowrap' }}>
              <Typography variant="body2" sx={{ fontWeight: 600, color: 'success.main', fontSize: '1.25rem' }}>
                {item.rating}
              </Typography>
  ...
  const steps = [
    {
      image: "/1-apply.svg",
      title: "Apply online",
      description: "Get an instant quote and submit your application in minutes."
    },
    {
      image: "/2-medical.svg",
      title: "Provide medical info",
      description: "Some applications require health questions or a brief follow-up. If needed, we'll schedule a free medical exam at your convenience."
    },
    {
      image: "/3-decision.svg",
      title: "Get a decision",
      description: "Once reviewed, you'll receive a decision. If approved, you get your coverage and a 30-day free-look period. QuickDecision℠ may offer faster approval, often without an exam."
    }
  ];
...
const CoverageOptionsSection: React.FC<{ categoryCards: any[] }> = ({ categoryCards }) => {
  const features = getClientFeatures();
  ...
  return (
    <Box sx={{ py: { xs: 4, md: 6 }, bgcolor: '#ecf7f4' }}>
      <Container sx={{ maxWidth: { lg: '1400px !important' } }}>
        <Stack spacing={4}>
          <Box>
            <Typography variant="h1" gutterBottom sx={{ fontSize: '36px', lineHeight: 1.1, fontWeight: 700 }}>
              Review your coverage options.
            </Typography>
            <Typography variant="body1" color="text.secondary" sx={{ fontSize: '16px', maxWidth: 600 }}>
          ...
          <Box sx={{ 
            ...
            gridTemplateColumns: { 
              xs: '1fr',
              lg: categoryCards.length === 1 ? '1fr' : categoryCards.length === 2 ? 'repeat(2, 1fr)' : categoryCards.length === 3 ? 'repeat(3, 1fr)' : 'repeat(4, 1fr)'
            },
            gap: 3,
            alignItems: 'flex-start'
          }}>
  ...
  <Box sx={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' }}>
    ...
    <Box sx={{ maxWidth: '600px' }}>
      <Stack spacing={3}>
        ...
        <Typography variant="body1" color="text.secondary" textAlign={"justify"}>
          For more than 50 years AMA-sponsored insurance has been protecting physicians and their families. As a subsidiary of the American Medical Association, AMA Insurance taps into the group buying power of over one million physicians to offer specially negotiated rates and tailored benefits from top insurance companies.
        </Typography>
      </Stack>
    </Box>
  </Box>
...
const AboutSection: React.FC = () => (
  ...
  >
    <Container maxWidth="xl">
      ...
      >
        {ACTIVE_CLIENT_ID === 'ama' && <AboutAMASection />}
        <AboutNYLSection />
      </Box>
    </Container>
  </Box>
  ...
  >
    <Container maxWidth="lg">
      <Stack spacing={2}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7 }}>
          <Typography component="span" sx={{ textDecoration: 'underline', cursor: 'pointer', color: 'primary.main' }}>
            Click here
          </Typography> for more information on this coverage—features, costs, eligibility, renewability, limitations, and exclusions.
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7 }}>
          AMA-sponsored insurance described on this page are underwritten by New York Life Insurance Company, 51 Madison Ave., New York, NY 10010 on Policy Form GMR. Coverage may not be available in all states and product features may vary by state.
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7 }}>
        ...
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7 }}>
          AMA incurs administrative expenses in connection with this program. To maintain these valuable benefits it is reimbursed for such expense.
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7, fontWeight: 600 }}>
          Disability Insurance with Step Rated Premiums:
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7 }}>
          Rates and premium credits are reviewed and may change annually. The premium credit is guaranteed for at least the first year of coverage.
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7 }}>
          Rates are based on your age on the effective date of coverage. Rates for the Disability Insurance with Step Rated Premiums are based on attained age on each renewal date. Depending on program experience, premiums may change on the policy anniversary date (July 1) or your annual renewal date (whichever is later).
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7, fontWeight: 600 }}>
          Disability Insurance with Step Rated Premiums:
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7 }}>
          Attention Residents of AK, AL, AZ, CA, FL, GA, HI, LA, MS, NM, OR, SC, TX and WA: Due to higher than national average claims experience in these states, rates are 40% higher in California and Florida, and 20% higher in Alaska, Alabama, Arizona, Georgia, Hawaii, Louisiana, Mississippi, New Mexico, Oregon, South Carolina, Texas and Washington. If you live in one of these states, the rates shown already reflect this premium adjustment.
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7, fontWeight: 600 }}>
          Level Term Life Insurance:
        </Typography>
        <Typography variant="body2" color="text.secondary" sx={{ fontSize: '0.875rem', lineHeight: 1.7 }}>
          The rate shown is the Super Preferred Risk Class rate for qualified non-tobacco users in good health and who meet additional underwriting guidelines.
        </Typography>
      </Stack>
    </Container>
  </Box>
